...
- macOS Apple Silicon：`Helios-X.Y.Z-mac-arm64.dmg`
- macOS Intel：`Helios-X.Y.Z-mac-x64.dmg`
- Windows x64：`Helios-X.Y.Z-windows-x64.exe`

安装包包含桌面后端、Eve 和 Node 运行时；Windows 额外包含 Portable Git。安装包文件名保留 Helios 前缀，以兼容现有客户端的自动更新匹配规则；macOS 应用为 Sealos.app。
